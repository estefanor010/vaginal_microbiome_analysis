...
function species_results = analyze_individual_species_link_percentages(vaginal_data, parameters)
...
%   species_results (struct): 
    ...
    subject_ids = unique(vaginal_data.("Subject ID"));
    ...
    for patient = 1:num_patients
        ...
        num_present_species = length(idx_species); %amount of species in a patient
        if num_present_species < 2
            continue;  % Skip patients with <2 species
        end
        
        % track all species at a global level on each patient 
        %this is only useful for filtering empty species at the end 
        ...
        for idx1 = 1:num_present_species
            ...
            % Possible links: (n_present_species - 1) per species on a
            % patient
            possible_links(current_species) = possible_links(current_species) + (num_present_species - 1);
            
            % check links with other species
            for idx2 = (idx1+1):num_present_species
                partner_species = idx_species(idx2);
                
                
                has_link = helpers.calculate_links(...
                    rel_abun(:,idx1), rel_abun(:,idx2), parameters);
                
                % track both species that have link e.g. a--b track a and b
    ...
    valid_species = possible_links > 0; %boolean 
    ...
    plots.plot_individual_species_percentage(species_results, parameters);
end
